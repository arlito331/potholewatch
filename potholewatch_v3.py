"""
PotholeWatch v3.0.8 — Multi-source road incident intelligence
============================================================
CHANGES vs v3.0.7:
  - incidents.json now stores FULL enriched data (article images, social posts,
    quotes, cross-ref evidence, score reasoning) — not just slim dedup metadata
  - Dashboard can now render all fields on click
  - sv_image_b64 removed (not used anymore)
"""
...
def save_inventory(inv):
    # Build flat incidents list for dashboard compatibility
    incidents_list = []
    for h, rec in inv["incidents"].items():
        # Ensure _key is set for dashboard
        rec_out = dict(rec)
...
Scoring levels:
  - CRITICAL: Article explicitly names pothole/bache/hueco as cause, OR strong social + institutional evidence
  - HIGH: Rollover/vuelco/volcado (multi-rollover = strong road surface suspicion) OR multiple citizen posts + MOP/ATTT records
  - MEDIUM: Some social posts OR institutional evidence (1-2 items) OR single-vehicle accident on known deteriorated road
  - LOW: Clear driver-only cause (drunk, phone, speeding confirmed) with zero road condition signals
...
# ============================================================
# 7. BUILD FULL INCIDENT RECORD (new — for JSON storage)
# ============================================================

def build_incident_record(case_id, inc, coords, social, crossref, score, scan_time_iso):
    """
    Build the full enriched record to store in incidents.json.
    Everything the dashboard needs to render a rich detail view.
    """
    # Best quote: first non-empty from social posts
    quote = ""
    ...
    # Best source URL for article
    article_url = inc.get("url", "")

    # Source name
    source_name = inc.get("source", "")

    # Collect all image URLs (article + citizen)
    ...
    return {
        ...
        "confidence": _score_to_confidence(score["score"], social_hits, len(mop) + len(attt)),
        "sources_confirmed": _confirmed_sources(inc, social, crossref),
...
<div style="max-width:720px;margin:auto;">
  ...
  <div style="text-align:center;font-size:11px;color:{DIM};padding:24px 0;border-top:1px solid {SOFT};margin-top:8px;">
    ...
    <div>PotholeWatch v3.0.8 — automated road incident intelligence.<br/>Next scan in 8 hours.</div>
  </div>
</div></body></html>"""
...
def main():
    ...
    print(f"=== PotholeWatch v3.0.8 scan @ {scan_time_iso} ===")
    ...
    for territory in TERRITORIES:
        ...
        for inc in incidents:
            ...
            # BUILD FULL RECORD for JSON storage
            full_record = build_incident_record(case_id, inc, coords, social, crossref, score, scan_time_iso)
