"""
PotholeWatch v5.4.0 — The Evidence Engine + Instagram via Apify
===============================================================
NEW IN v5.4:
  ...
  - DIRECT POST URL SCRAPING: also searches Google for direct
    Instagram post URLs from @traficopanama mentioning the location,
    then passes those directly to Apify
...
INSTAGRAM_TRAFFIC_ACCOUNT = "traficopanama"
...
def scrape_instagram_for_incident(incident):
    """
    Two-pronged Instagram search via Apify:
    1. Scrape news account profiles for posts matching this incident location
    2. Search for direct @traficopanama post URLs via Claude web search
    """
    ...
    # ── PRONG 1: Scrape news account profiles ──────────────
    ...
    # ── PRONG 2: Find @traficopanama direct post URLs ──────
    print(f"    [Apify] searching for @traficopanama posts about '{loc_short}'...")
    try:
        search_prompt = f"""{DATE_ANCHOR}

Search Instagram for recent posts from @traficopanama about this accident location.
...
Run these searches:
1. site:instagram.com/p/ "traficopanama" "{loc_short}"
2. site:instagram.com "traficopanama" "{loc_short}" accidente
3. instagram.com traficopanama "{loc_short}" {date_str}

Return a JSON object (no prose, no fences):
{{"post_urls": ["https://www.instagram.com/p/SHORTCODE/", ...]}}

RULES:
- Only include URLs in format https://www.instagram.com/p/[SHORTCODE]/
- Shortcode must be alphanumeric, 5-15 chars
- Never guess or construct URLs
- Empty array if nothing found

JSON only."""
        raw = claude_call(search_prompt, tools=[WEB_SEARCH_TOOL], max_tokens=500)
        parsed = extract_json_object(raw)
        direct_urls = []
        if parsed and parsed.get("post_urls"):
            for u in parsed["post_urls"]:
                if is_valid_url(u) and "instagram.com/p/" in u:
                    direct_urls.append(u)

        if direct_urls:
            print(f"    [Apify] found {len(direct_urls)} direct traficopanama URLs, scraping...")
            run_input2 = {
            ...
            direct_posts = apify_run_and_wait(APIFY_INSTAGRAM_ACTOR, run_input2, timeout=60)
            print(f"    [Apify] got {len(direct_posts)} posts from direct URLs")
            all_posts.extend(direct_posts)
        else:
            print(f"    [Apify] no direct traficopanama URLs found")
    except Exception as e:
        print(f"    [Apify] traficopanama search error: {e}")
...
def main():
    ...
    for territory in TERRITORIES:
        ...
        for inc in incidents:
            ...
            # Merge web + Instagram
            all_posts    = web_data.get("social_posts",[])    + ig_posts
            all_comments = web_data.get("keyword_comments",[]) + ig_comments
            ...
            print(f"    posts: {len(all_posts)} ({valid_post_links} w/ valid URLs) | comments: {mention_count} ({valid_comment_links} w/ links)")
