"""
PotholeWatch v5.2.0 — The Evidence Engine
==========================================
NEW IN v5.2:
  - LOOKBACK back to 8 days (was 30 for catch-up run)
  - SEVERITY TIERS based purely on mention count:
      1 mention  = MEDIUM
      2 mentions = HIGH
      3+ mentions = CRITICAL
    No AI scoring. The citizen count IS the evidence.
  - SOCIAL POST FINDER: for each incident, find the news outlet's
    exact Facebook/Instagram/X post of that article, then extract
    any keyword-matching comments from that post
  - GEOLOCATION LINK: every case gets a clickable Google Maps link
  - CLICKABLE LINKS: all sources, posts, and comments are hyperlinked
    throughout email and dashboard
"""
...
# === TUNABLE KNOBS ===========================================
LOOKBACK_DAYS = 8       # <<< back to 8 days (was 30 for catch-up)
REALERT_THRESHOLD = 3   # re-surface a known case if mentions grow by >= this
# Severity tiers based purely on mention count
SEVERITY_CRITICAL = 3   # 3+ mentions = CRITICAL
SEVERITY_HIGH = 2        # 2 mentions = HIGH
SEVERITY_MEDIUM = 1      # 1 mention = MEDIUM
# =============================================================
...
# ============================================================
# 2. FIND SOCIAL POSTS + KEYWORD COMMENTS
# ============================================================

def find_social_posts_and_comments(incident):
    """
    Find the news outlet's exact social media post of this article,
    then extract any keyword-matching comments from that post.
    """
...
PART 1 — Find where this specific news article was posted on social media.
PART 2 — Find any citizen comments on those posts that mention road conditions.
...
PART 1 SEARCHES — Find the outlet's social posts of this exact article:
1. Search: 'site:facebook.com "{source}" "{loc_short}" accidente {date_str}'
2. Search: 'site:facebook.com "{title[:50]}"'
3. Search: 'site:instagram.com "{source}" "{loc_short}" accidente'
4. Search: 'site:twitter.com OR site:x.com "{source}" "{loc_short}" accidente {date_str}'
5. Search: '{source} facebook "{loc_short}" accidente {date_str}'

PART 2 SEARCHES — Find keyword comments on those posts:
6. For each social post URL found, try to fetch it and extract comments containing the road keywords
7. Search: '"{loc_short}" bache OR hueco OR "mal estado" facebook comment {date_str}'
8. Search: '"{loc_short}" bache OR hueco site:facebook.com'
9. Search: '"{loc_short}" bache OR hueco site:twitter.com OR site:x.com'
10. Search: '"{loc_short}" deterioro vial MOP hueco accidente'
...
{{
  "social_posts": [
    {{
      ...
      "outlet": "TVN|Telemetro|La Prensa|etc",
      "post_url": "direct URL to the social media post",
      "post_caption": "the text of the post if visible",
  ...
  "keyword_comments": [
    {{
      ...
      "source_name": "outlet page name or username",
      "comment_url": "direct permalink to the comment if available",
      "post_url": "URL of the post this comment is under",
      "quote": "the exact comment text containing road keywords",
      "keywords_matched": ["bache","hueco",...],
      "date": "YYYY-MM-DD if known"
    }}
  ],
  "summary": "1-2 sentence neutral summary of what citizens are saying about this road's condition. Present only — no conclusions about accident cause."
}}

CRITICAL RULES:
- social_posts must be the OUTLET'S OWN post of THIS specific article — not random posts
- keyword_comments must contain at least one road-condition keyword
- Quotes must be REAL verbatim text — never invent
- comment_url should be a direct permalink to that specific comment if possible
- If nothing found for either section, return empty arrays
- Keep summary neutral — no verdicts
...
JSON only."""
    ...
    parsed.setdefault("social_posts", [])
    parsed.setdefault("keyword_comments", [])
    parsed.setdefault("summary", "")
    return parsed

# ============================================================
# 3. LIGHTWEIGHT GEOCODE
# ============================================================
...
def mention_signature(m):
    ...
    q = re.sub(r"\s+", " ", q)[:120]
    url = (m.get("comment_url","") or m.get("url","") or "").strip().lower()
...
def _social_post_row(p):
    ...
    link = f'<a href="{url}" style="color:{ACCENT};text-decoration:none;font-weight:700;">→ Open {platform} post</a>' if url else ""
    src = f"{platform}" + (f" · {outlet}" if outlet else "") + (f" · {date}" if date else "")
...
def _comment_row(m):
    ...
    # prefer direct comment link, fall back to post link
    link_url = comment_url or post_url
    link = f' <a href="{link_url}" style="color:{ACCENT};text-decoration:none;font-weight:700;">→ ver comentario</a>' if link_url else ""
...
def build_card(case, new_count, is_new_case, severity):
    ...
    social_html = ""
    if social_posts:
        social_html += f'<div style="font-size:10px;letter-spacing:2px;color:{ACCENT};font-weight:700;text-transform:uppercase;margin:14px 0 8px;">News outlet social posts — click to read comments</div>'
        social_html += "".join(_social_post_row(p) for p in social_posts[:4])

    comments_html = ""
    if keyword_comments:
        comments_html += f'<div style="font-size:10px;letter-spacing:2px;color:{ACCENT};font-weight:700;text-transform:uppercase;margin:14px 0 8px;">Citizen comments mentioning road conditions ({len(keyword_comments)})</div>'
    ...
    count = len(case.get("keyword_comments", []))
    tag = ""
    if is_new_case:
    ...
    if case.get("lat") and case.get("lng"):
        ml = maps_link(case["lat"], case["lng"])
        maps_html = f'<a href="{ml}" style="display:inline-block;margin-top:8px;font-size:11px;color:{ACCENT};text-decoration:none;border-bottom:1px solid rgba(217,79,43,0.3);">📍 Open in Google Maps →</a>'
...
<div style="background:{CARD_BG};border-radius:8px;padding:24px;margin-bottom:20px;border-left:4px solid {color};">
  <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
    ...
    <span style="font-size:10px;letter-spacing:2px;color:{DIM};">·</span>
  ...
  <div style="background:{SOFT};padding:16px;border-radius:6px;margin:12px 0;">
    ...
    <div style="margin-top:12px;"><a href="{case.get('url','')}" style="display:inline-block;background:{ACCENT};color:{TEXT};padding:10px 18px;border-radius:4px;font-size:12px;text-decoration:none;font-weight:700;letter-spacing:1px;">READ FULL ARTICLE →</a></div>
  </div>
...
<div style="max-width:720px;margin:auto;">
  <div style="margin-bottom:24px;padding:24px;background:{CARD_BG};border-radius:8px;border-top:4px solid {ACCENT};">
    <div style="font-size:11px;letter-spacing:4px;color:{ACCENT};font-weight:700;">POTHOLEWATCH · THE EVIDENCE ENGINE · v5.2</div>
  ...
  <div style="text-align:center;font-size:11px;color:{DIM};padding:24px 0;border-top:1px solid {SOFT};margin-top:8px;">
    ...
    <div>PotholeWatch v5.2 — The Evidence Engine.<br/>Accident + citizen testimony + geolocation. You decide.</div>
  </div>
</div></body></html>"""
...
def main():
    ...
    print(f"=== PotholeWatch v5.2.0 (Evidence Engine) @ {scan_time_iso} ===")
    ...
    cards = []
    pending_cards = []  # (case, new_count, is_new, severity)
    ...
    for territory in TERRITORIES:
        ...
        for inc in incidents:
            ...
            print(f"    social posts found: {len(social_posts)} | keyword comments: {mention_count}")
            ...
            if existing_case:
                ...
                # merge social posts (dedup by URL)
    ...
    # Sort: CRITICAL first, then HIGH, then NEW cases, then by count
    ...
    sev_label = ""
    if n_critical: sev_label = f"{n_critical} CRITICAL"
    elif n_high: sev_label = f"{n_high} HIGH"
    else: sev_label = f"{n_medium} MEDIUM"
