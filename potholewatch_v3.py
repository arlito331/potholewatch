"""
PotholeWatch v3.0.1 — Multi-source road incident intelligence
============================================================
Pipeline per scan:
...
def load_inventory():
    ...
    with open(INVENTORY_FILE) as f:
        inv = json.load(f)
    # Backfill keys for older inventory files (v2 didn't have counter)
...
def claude_vision(image_url, prompt):
    """Send an image URL to Claude for road-damage detection."""
    try:
...
Only road accidents. JSONL only — no preamble, no fences."""
    ...
    raw = claude_call(prompt, tools=tools, max_tokens=4000)
    incidents = []
...
def street_view_urls(lat, lng):
    """4 directional Street View images at the incident coords."""
    ...
    for heading in [0, 90, 180, 270]:
        params = {
            ...
            "location": f"{lat},{lng}",
            "heading": heading,
            "pitch": -20,  # tilt down to see road surface
            "fov": 80,
...
def vision_filter_streetview(urls):
    """Run each Street View image through Claude vision — keep only those showing road damage."""
    hits = []
    for url in urls:
...
<p style="font-size:11px;color:#888;">
  <strong>PotholeWatch v3.0.1</strong> — automated road incident intelligence for PowerFix.<br/>
...
def main():
    print(f"=== PotholeWatch v3.0.1 scan @ {datetime.utcnow().isoformat()}Z ===")
    inv = load_inventory()
